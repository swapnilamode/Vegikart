    ...
    const handleSubmit = async(e) => {
        ...
        if(firstName && email && password && confirmpassword){
            if(password === confirmpassword){
                const fetchdata = await fetch(`${process.env.REACT_APP_SERVER_DOMIN}/vsignup`,{
                    method : "POST",
                    headers : {
    ...
    return (
        <div className="p-3 md:py-8">
            <div className="w-full max-w-sm bg-white m-auto flex-col p-4">
                ...
                <p className="text-left text-sm -mt-2">Already have an account ?{""} <Link to={"/vlogin"} className="text-red-500 underline">Vendor Login</Link></p>
            </div>
        </div>
